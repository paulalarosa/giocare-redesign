...
Object.entries(SEMANA).forEach(([dia, slots]) => {
  ...
  slots.forEach(([ini, fim, ativo]) => {
    ...
    linha.innerHTML = `
      ...
      <div class="acts">
        <button type="button" class="alavanca" role="switch" aria-checked="${ativo}" aria-label="Oferecer o horário de ${ini} às ${fim} na ${dia}"><i></i></button>
...
wrap.addEventListener('click', (e) => {
  ...
  const alternar = e.target.closest('.alavanca');
  if (!alternar) return;
  ...
  alternar.setAttribute('aria-checked', String(!desligando));
  if (desligando) window.gioToast('Horário desativado. Some da agenda a partir de amanhã.');
});
...
document.addEventListener('click', (e) => {
  ...
  if (b && b.dataset.acao === 'add-horario') {
    window.gioToast('Novo horário criado.');
  }
});
